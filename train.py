...
def create_datasets(args):
    ...
    train_data = DatasetImageCoord(
        args.data_path, args.train_csv_path, args.pos_csv_path
    )
...
def train_epoch(args, epoch, model, data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        # Return kspace also we can ignore that for train and test
...
def create_arg_parser():
    ...
    parser.add_argument(
        "--num-chans", type=int, default=32, help="Number of U-Net channels"
    )
    parser.add_argument("--batch-size", default=2, type=int, help="Mini batch size")
    parser.add_argument(
        "--num-epochs", type=int, default=150, help="Number of training epochs"
    )
